...
public class ZRLEDecoder extends ZlibDecoder {
	...
	public ZRLEDecoder() { logger = Logger.getLogger("com.glavsoft.rfb.encoding"); }
	...
	private int decodePlainRle(byte[] bytes, int offset, Renderer renderer,
		...
		while (decodedOffset < decodedEnd) {
			int color = renderer.readCompactPixelColor(bytes, index);
			index += bytesPerCPixel;
			int rlength = 1;
			do {
				rlength += bytes[index] & 0x0ff;
			} while ((bytes[index++] & 0x0ff) == 255);
			assert rlength * bytesPerCPixel <= decodedEnd - decodedOffset;
	...
	private int decodePaletteRle(byte[] bytes, int offset, Renderer renderer,
		...
		while (decodedOffset < decodedEnd) {
			...
			if (rlength * bytesPerCPixel > decodedEnd - decodedOffset) {
				logger.severe("rlength: " +rlength + " must be: " + (decodedEnd - decodedOffset) / bytesPerCPixel);
	...
	private int decodePacked(byte[] bytes, int offset, Renderer renderer,
		...
		for (int i = 0; i < tileHeight; ++i) {
			int decodedRowEnd = decodedOffset + tileWidth * renderer.getBytesPerPixelSignificant();
			int byteProcessed = 0;
			int bitsRemain = 0;
			...
			while (decodedOffset < decodedRowEnd) {
				...
				decodedOffset += renderer.putPixelIntoByteArray(decodedBytes, decodedOffset, color);
			}
		}
		renderer.drawBytes(decodedBytes, 0, tileX, tileY, tileWidth, tileHeight, true);
	...
	private int readPalette(byte[] bytes, int offset, Renderer renderer,
		...
		for (int i=0; i<paletteSize; ++i) {
			palette[i] = renderer.readCompactPixelColor(bytes,
